...
export function EmailLabel(props: EmailLabelProps) {
  ...
  return (
    <label className={clsx({ [styles.error]: error })} htmlFor="email">
      <span className={styles.parts}>Email</span>
      <EmailLabelText {...{ text }} />
    </label>
  );
}
...
export function EmailLabelText({ text }: { text: string }) {
  ...
  return (
    <SwitchTransition>
      ...
      >
        <span className={clsx(styles.parts, { [styles.partsFocused]: !text })} ref={nodeRef}>
          {text}
        </span>
      </CSSTransition>
    </SwitchTransition>
...
export function genLabel({ focused, error }: EmailLabelProps) {
  ...
  return 'me when I can access app';
}
